...
class PromptVisitor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        match node.body:
            case [ast.Expr(value=ast.Constant(s)), ast.Pass()] if type(s) == str:
                ...
                self.state = "complete"
            case lst if any(
                type(nd) is ast.Expr and type(nd.value) is ast.Constant and type(nd.value.value) is str
                for nd in lst
            ):
                # 取出第一个匹配的字符串
                self.description = next(
                    nd.value.value
                    for nd in lst
                    if type(nd) is ast.Expr and type(nd.value) is ast.Constant and type(nd.value.value) is str
                )
                self.state = "complete"
            case _other:
                self.state = "error"
    ...
    def translate_func_decl(self, doctest_transformation: str) -> str | None:
        ...
        return self.translator.translate_function_signature(self.name, self.args, self.returns, desc)
...
def translate_tests(
    ...
    match tests_ast:
        ...
        case _other:
            ...
            body_ast = tests_ast.body
            body_ast = [stmt for f in body_ast if isinstance(f, ast.FunctionDef) for stmt in f.body]
            # body_ast = [item for b in body_ast if isinstance(b, ast.FunctionDef) for item in (b.body if isinstance(b.body, list) else [b.body])]
