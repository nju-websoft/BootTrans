...
def cached_eval_script(problem, index, lang) -> dict:
    # racket swift
    # program = problem["prompt"] + problem["completions"][index] + '\n' + problem["tests"]
    # bash\swift 需要problem["tests"].lstrip('}'),scala lstrip('    }')
    # print(lang)
    if lang == 'scala':
        completion = problem["completions"][index]
        if 'def main' in completion:
            completion = completion.split('def main')[0]
        else:
            completion = completion.rstrip('}')
        program = completion + '\n' + problem["tests"].lstrip('    }')
    elif lang == 'java':
        
        completion = problem["completions"][index]
        if 'public static void main' in completion:
            completion = completion.split('public static void main')[0]
        program = completion + '\n' + problem["tests"].lstrip('    }')

    elif lang == 'cpp':
        completion = problem["completions"][index]
        if 'int main' in completion:
            completion = completion.split('int main')[0]
        if 'using namespace std;' not in completion:
...
def evaluate_problem(output_dir: Path, problem_json_path: Path, max_workers: int, input_dir: Path = None, lang: str = None):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for j in executor.map(lambda index: cached_eval_script(problem, index, lang), range(min_problem, num_problems)):
            test_results["results"].append(j)
            with open_json(test_results_path, "w") as f:
                f.write(json.dumps(test_results, indent=2))

def evaluate_correctness(output_dir: Path, problem, max_workers: int, lang: str):
    '''
        Modify by yhwu
    '''
    if len(problem["completions"]) == 0:
    ...
    suffixes = '.json'
    test_results_path = output_dir / (problem['name'] + suffixes)
    test_results_path.parent.mkdir(mode=0o755, parents=True, exist_ok=True)
    ...
    if len(test_results["results"]) == num_problems:
        ...
        print(f"ERROR more results than completions")
        return
...
def main():
    ...
    args.add_argument("--lang", type=str, help="The language to evaluate")
    ...
    if args.file:
        ...
        if args.job_file is not None or args.job_file_line is not None:
            ...
            exit(2)
        evaluate_problem(args.output_dir, Path(args.file), args.max_workers, args.lang)
    elif args.dir:
        if args.output_dir is None:
        ...
        with open(args.dir, 'r') as f:
            for l in tqdm(f):
                line = json.loads(l)
                # files.append(line)
                evaluate_correctness(args.output_dir, line, args.max_workers, args.lang)
