...
def eval_string_script(language, program, temp_dir='./temp_dir'):
    if language in EVALUATORS:
        ...
        file_ext = f"_test.{language}" if language != "go_test.go" else "_test.go"
        
        temp_dir = os.path.join(temp_dir, str(uuid.uuid4()))
        if not os.path.exists(temp_dir):
            os.makedirs(temp_dir)
        
    with tempfile.NamedTemporaryFile(suffix=file_ext, dir=temp_dir, delete=True) as f:
