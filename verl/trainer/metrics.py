...
def compute_data_num_metrics(batch: DataProto) -> dict[str, Any]:
    source_languages = batch.non_tensor_batch['source_language']
    target_languages = batch.non_tensor_batch['target_language']
    data_nums = {}
    
    for src_lang, tgt_lang in zip(source_languages, target_languages):
        key = f'data_num/{src_lang}-{tgt_lang}'
        if key in data_nums:
            data_nums[key] += 1
        else:
            data_nums[key] = 1
...
# def save_problems(batch: DataProto):
