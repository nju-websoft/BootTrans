...
@register_adv_estimator(AdvantageEstimator.GRPO_LANG)
def compute_grpo_outcome_advantage_language(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: torch.Tensor,
    eps: float = 1e-6,
    ...
    **kwargs,
    ...
    Returns:
        advantages: `(torch.Tensor)` normalized per-uid, shape (bs, response_length)
        intra_advantages: `(torch.Tensor)` sibling weight B/(A+B), shape (bs,)

    """
    scores_raw = token_level_rewards.sum(dim=-1)
    scores = scores_raw.clone()
    id2score = defaultdict(list)
    id2mean, id2std, id2sum = {}, {}, {}
    ...
    parent_index = kwargs.get("parent_uid")
    if parent_index is None:
        raise RuntimeError("parent_uid is required for GRPO_LANG advantage.")
    if len(parent_index) != bsz:
        raise RuntimeError(f"parent_uid length {len(parent_index)} != batch size {bsz}.")

    parent2score, parent2sum = defaultdict(list), {}

    for i in range(bsz):
        parent_key = parent_index[i].item() if hasattr(parent_index[i], "item") else parent_index[i]
        parent2score[str(parent_key)].append(scores_raw[i])

    for idx in parent2score:
        assert len(parent2score[idx]) > 1, "GRPO needs rollout.n > 1."
        parent2sum[idx] = torch.sum(torch.tensor(parent2score[idx]))
        
    intra_advantages = scores.new_zeros(bsz) # 确保类型正确

    for i in range(bsz):
        current_group_reward_sum = id2sum[index[i]]
        parent_key = parent_index[i].item() if hasattr(parent_index[i], "item") else parent_index[i]
        all_parent_reward_sum = parent2sum[str(parent_key)]

        A = torch.clamp(current_group_reward_sum, min=R_min)
        B = torch.clamp(all_parent_reward_sum - current_group_reward_sum, min=R_min)
        intra_advantages[i] = B / (A + B + eps)

    intra_advantages = intra_advantages.unsqueeze(-1) * response_mask
    return advantages, intra_advantages
...
def average_loss(
    values: torch.Tensor, mask: torch.Tensor, mode: Literal["token", "seq"], eps: float = 1e-8, weights=None
) -> torch.Tensor:
    ...
    if mode == "token":
        ...
        if weights is None:
            raise ValueError("Mode 'weighted' requires 'weights' tensor to be provided.")

        # Reduce token-level weights (bs, seq_len) to sequence weights (bs,)
        if weights.dim() == values.dim():
            weights = VF.masked_mean(weights, mask, dim=-1)
        # Squeeze trailing singleton dim if provided as (bs, 1)
        if weights.dim() == values.dim() and weights.shape[-1] == 1:
            weights = weights.squeeze(-1)

        sequence_means = (values * mask).sum(-1) / (mask.sum(-1) + eps)

        # Correctly calculates the weighted average: (Sum of weighted means) / (Sum of weights)
        return (sequence_means * weights).sum() / (weights.sum() + eps)
    else:
...
def compute_policy_loss(
    ...
    loss_avg_mode: Literal["token", "seq", "weighted"],
    ...
    **kwargs,
    ...
    if loss_avg_mode == "weighted":
        metrics["entropy_loss"] = average_loss(-log_probs, response_mask, mode=loss_avg_mode, weights=weights)
    else:
    ...
    if loss_avg_mode == 'weighted':
        final_pg_loss = average_loss(final_pg_loss, response_mask, mode=loss_avg_mode, weights=weights)
    else:
