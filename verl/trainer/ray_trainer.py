...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0):
    ...
    if "parent_uids" in data.non_tensor_batch:
        adv_inputs.update({"parent_uid": data.non_tensor_batch["parent_uids"]})
...
class RayPPOTrainer:
    ...
    def __init__(
        self,
        config: Union[PPOConfig, PPOConfigForTranslation],
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            # MODIFY
            test_batch.non_tensor_batch["val"] = np.ones(len(test_batch), dtype=object)
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            # MODIFY
            metrics.update(compute_data_num_metrics(batch=batch))
            for logger in self.logger.loggers:
                logger.data_log(batch=batch)
