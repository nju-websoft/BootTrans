...
class Logger(ABC):
    ...
    @abstractmethod
    def data_log(self, data: dict[str, Any], step: int) -> None: ...
...
class FileLogger(Logger):
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        with open(os.path.join(config["trainer"]["save_checkpoint_path"], "train_data.csv"), "w") as f:
            pass
    ...
    def data_log(self, batch):
        question_ids = batch.non_tensor_batch['question_id']
        source_languages = batch.non_tensor_batch['source_language']
        target_languages = batch.non_tensor_batch['target_language']
        counter = Counter(zip(question_ids, source_languages, target_languages))
        output_file = Path(os.path.join(self.config["trainer"]["save_checkpoint_path"], "train_data.csv"))
        
        output_file.write_text(
            "".join(
                f"{qid},{src_lang},{tgt_lang},{count}\n"
                for (qid, src_lang, tgt_lang), count in counter.items()
            )
        )
